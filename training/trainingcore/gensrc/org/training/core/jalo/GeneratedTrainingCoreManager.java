...
import org.training.core.jalo.MyTrainingProcess;
import org.training.core.jalo.TrainingEmailProcess;
import org.training.core.jalo.TrainingOldCartRemovalJob;
import org.training.core.jalo.components.TrainingVideoComponent;
...
public abstract class GeneratedTrainingCoreManager extends Extension
{
	...
	public MyTrainingProcess createMyTrainingProcess(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.MYTRAININGPROCESS );
			return (MyTrainingProcess)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating MyTrainingProcess : "+e.getMessage(), 0 );
		}
	}
	
	public MyTrainingProcess createMyTrainingProcess(final Map attributeValues)
	{
		return createMyTrainingProcess( getSession().getSessionContext(), attributeValues );
	}
	...
	public TrainingOldCartRemovalJob createTrainingOldCartRemovalJob(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.TRAININGOLDCARTREMOVALJOB );
